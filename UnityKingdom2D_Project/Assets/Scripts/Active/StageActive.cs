...
public class StageActive : MonoBehaviour
{
    ...
    private void GenerateTier_1(Transform spawner, string tag)
    {
    ...
    private void GenerateTier_2(Transform spawner, string tag)
    {
        var generator = spawner.GetComponent<GenerateActive>();
        for (int i = 0; i < generator.Creeps.Length; i++)
        {
    ...
    private void GenerateTier_3(Transform spawner, string tag)
    {
        var generator = spawner.GetComponent<GenerateActive>();
        for (int i = 0; i < generator.Creeps.Length; i++)
        {
    ...
    private void LateUpdate()
    {
        ...
        {
            StageSetup(LevelSet.Hard, GenerateTier_3, 7, 5);
        }
        else if (manager.KillPoint > 30 && manager.KillPoint <= 99 && level != LevelSet.Normal)
        {
            StageSetup(LevelSet.Normal, GenerateTier_2, 5, 3);
        }
        else if (manager.KillPoint <= 30 && level != LevelSet.Easy)
        {
            StageSetup(LevelSet.Easy, GenerateTier_1, 3, 1);
        }
    }
}
